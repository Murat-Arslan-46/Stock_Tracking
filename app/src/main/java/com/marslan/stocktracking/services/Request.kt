...
class Request @Inject constructor(){
    ...
    private fun getOrderRef() = reference.child(rootReference).child("order")
    ...
    fun observeOrderMap(listener: ValueEventListener) = getOrderRef().addValueEventListener(listener)

    fun addCustomer(id: String, value: Customer) = getCustomerRef().child(id).setValue(value)

    fun addProduct(id: String, value: Product) = getProductRef().child(id).setValue(value)

    fun addInvoice(id: String, value: Invoice) = getInvoiceRef().child(id).setValue(value)

    fun addOrder(id: String, value: Order) = getOrderRef().child(id).setValue(value)
}